...
struct proc_dir_entry * proc_hq_dbg_mkdir1(const char *name)
{
...
EXPORT_SYMBOL(proc_hq_dbg_mkdir1);
...
*/
int proc_hq_dbg_add1(char * name,struct hq_dbg_entry hq_debug[],int size)
{
	...
	if(proc_dir==NULL)
	{
		printk(KERN_ERR "Create dir  hq-accdet/%s/ error !",name);
		return -1;
	}
	for(i=0;i<size;i++) 
	{
...
EXPORT_SYMBOL(proc_hq_dbg_add1);

/*
 * Called on initialization to mkdir /proc/hq-dbg/
 */
static int  __init proc_hq_debug_init1(void)
{
	proc_hq_debug_dir = proc_mkdir("hq-accdet", NULL);
	if (proc_hq_debug_dir == NULL)
	{
	    printk(KERN_ERR "chz Create hq-accdet/ error !\n");
			return -1;
	}
	return 0;
}
module_init(proc_hq_debug_init1);
