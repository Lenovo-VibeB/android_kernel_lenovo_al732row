...
#if 1 //chencheng
/*****speaker_gpio begin*******************************/
static int speaker_gpio_probe(struct platform_device *pdev);
static int speaker_gpio_remove(struct platform_device *pdev);

struct pinctrl *speaker_gpio_ctrl = NULL;
struct pinctrl_state *speaker_gpio_spkamp_mode0_pull_enable = NULL;
struct pinctrl_state *speaker_gpio_spkamp_l = NULL;
struct pinctrl_state *speaker_gpio_spkamp_h = NULL;


static const struct of_device_id speaker_gpio_of_match[] = {
	{.compatible = "mediatek,speaker_gpio",},
	{}
};

static struct platform_driver speaker_gpio_driver = {
	.probe	  = speaker_gpio_probe,
	.remove	 = speaker_gpio_remove,
	.driver = {
		.name  = "speaker_gpio",
	#ifdef CONFIG_OF
		.of_match_table = speaker_gpio_of_match,
	#endif
	}
};

int speaker_get_gpio_info(struct platform_device *pdev)
{
	int ret;

	speaker_gpio_ctrl = devm_pinctrl_get(&pdev->dev);
	if (IS_ERR(speaker_gpio_ctrl)) {
		dev_err(&pdev->dev, "Cannot find speaker gpio pinctrl!");
		ret = PTR_ERR(speaker_gpio_ctrl);
	}
	/*Cam0 Power/Rst Ping initialization */
	speaker_gpio_spkamp_mode0_pull_enable = pinctrl_lookup_state(speaker_gpio_ctrl, "speaker_gpio_spkamp_mode0_pull_enable");
	if (IS_ERR(speaker_gpio_spkamp_mode0_pull_enable)) {
		ret = PTR_ERR(speaker_gpio_spkamp_mode0_pull_enable);
		pr_debug("%s : pinctrl err, speaker_gpio_spkamp_mode0_pull_enable \n", __func__);
	}
	speaker_gpio_spkamp_l = pinctrl_lookup_state(speaker_gpio_ctrl, "speaker_gpio_spkamp_l");
	if (IS_ERR(speaker_gpio_spkamp_l)) {
		ret = PTR_ERR(speaker_gpio_spkamp_l);
		pr_debug("%s : pinctrl err, speaker_gpio_spkamp_l\n", __func__);
	}

	speaker_gpio_spkamp_h = pinctrl_lookup_state(speaker_gpio_ctrl, "speaker_gpio_spkamp_h");
	if (IS_ERR(speaker_gpio_spkamp_h)) {
		ret = PTR_ERR(speaker_gpio_spkamp_h);
		pr_debug("%s : pinctrl err, speaker_gpio_spkamp_h\n", __func__);
	}
	printk("[speaker_gpio%d] speaker_gpio_pinctrl----------\n", pdev->id);
	return 0;
}
...
static int __init speaker_gpio_device_init(void)
{
	printk("MediaTek speaker gpio driver init\n");
	if (platform_driver_register(&speaker_gpio_driver) != 0) {
		printk("unable to register speaker gpio driver.\n");
		return -1;
	}
	return 0;
}
...
static void __exit speaker_gpio_device_exit(void)
{
	printk("MediaTek touch panel driver exit\n");
	/* input_unregister_device(tpd->dev); */
	platform_driver_unregister(&speaker_gpio_driver);
}

late_initcall(speaker_gpio_device_init);
module_exit(speaker_gpio_device_exit);

MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("MediaTek speaker gpio driver");
...
bool OpenHeadPhoneImpedanceSetting(bool bEnable)
{
	...
	if (bEnable == true) {
		...
		Ana_Set_Reg(AUDDEC_ANA_CON6, 0x0841, 0xffff);
		/* Enable cap-less HC LDO (1.5V) & LDO VA33REFGEN */
...
//wangguan add double speaker begin 20150209
#ifdef  CONFIG_HQ_DAUL_SPEAKER
#ifdef  AW8736_MODE_CTRL
...
/*wangguan add double speaker begin 20150209*/
		...
		//udelay(1000);
		//mt_set_gpio_dir(GPIO_EXT_SPKAMP_EN_PIN, GPIO_DIR_OUT);	/* output */
                printk("wangguan Ext_Speaker_Amp_Change ON set GPIO \n");
                pinctrl_select_state(speaker_gpio_ctrl, speaker_gpio_spkamp_mode0_pull_enable);
                udelay(1000);

#ifdef AW8736_MODE_CTRL
        AW8736_MODE1();
#else
        pinctrl_select_state(speaker_gpio_ctrl, speaker_gpio_spkamp_h);	/* high enable */
#endif

        msleep(SPK_WARM_UP_TIME);
#endif
//wangguan add double speaker end 20150209  
...
#ifdef CONFIG_HQ_DAUL_SPEAKER
	//mt_set_gpio_mode(GPIO_EXT_SPKAMP_EN_PIN, GPIO_MODE_00); //GPIO117: DPI_D3, mode 0
        pinctrl_select_state(speaker_gpio_ctrl, speaker_gpio_spkamp_l); // low disbale
        udelay(500);
#endif
//wangguan add double speaker end 20150209
