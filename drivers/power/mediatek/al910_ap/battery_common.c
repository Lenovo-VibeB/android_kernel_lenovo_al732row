...
//add by chenlei2 for debug battery temperature
int g_battery_test_temperature=0;
int g_battery_test_enable=0;
...
/* 20160504 Add for lenovo MMI test by liunianliang */
#ifdef LENOVO_MMITEST_SUPPORT
	int chg_current;
	int lenovo_enable_charging;
#endif
/* end */
	/* Dual battery */
	int status_smb;
...
/* 20160504 Add for lenovo MMI test by liunianliang */
	...
	POWER_SUPPLY_PROP_lenovo_enable_charging,
#endif
/* end */
	/* Dual battery */
				...
				enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_batt_vol:
		 //huangyisong_add for_runin battery test start 	»       »       val->intval = data->BAT_batt_vol;	702
...
/* 20160504 Add for lenovo MMI test by liunianliang */
	...
	case POWER_SUPPLY_PROP_chg_current:
		val->intval = data->chg_current;
		break;
	case POWER_SUPPLY_PROP_lenovo_enable_charging:
		val->intval = data->lenovo_enable_charging;
		break;
#endif
/* end */
	default:
...
/* 20160504 Add for lenovo MMI test by liunianliang */
#ifdef LENOVO_MMITEST_SUPPORT
static int node_is_writeable(struct power_supply *psy,enum power_supply_property psp) {
	int ret = 1;

	switch (psp) {
	case POWER_SUPPLY_PROP_lenovo_enable_charging:
		battery_log(BAT_LOG_CRTI, "[node_is_writeable]  POWER_SUPPLY_PROP_lenovo_enable_charging is writeable\n");
		break;
	default:
		ret = -ENOENT;
		break;
				...
				enum power_supply_property psp, const union power_supply_propval *val)
{
	...
	unsigned int charging_enable = false;
	struct battery_data *data = container_of(psy, struct battery_data, psy);
	battery_log(BAT_LOG_CRTI, "[battery_set_property]  val->intval  = %d\n",val->intval);
	switch (psp) {
	case POWER_SUPPLY_PROP_lenovo_enable_charging:
		data->lenovo_enable_charging = val->intval;
		if (val->intval == 1) {
			charging_enable = true;
			cmd_discharging = 0;
			adjust_power = -1;
			battery_log(BAT_LOG_CRTI, "[battery_set_property]  enable chargeing !\n");
			battery_charging_control(CHARGING_CMD_ENABLE, &charging_enable);
		} else if (val->intval == 0) {
			charging_enable = false;
			adjust_power = -1;
			cmd_discharging = 1;
			battery_log(BAT_LOG_CRTI, "[battery_set_property]  disable chargeing !\n");
			battery_charging_control(CHARGING_CMD_ENABLE, &charging_enable);
		}
		break;
	default:
...
/* 20160504 Add for lenovo MMI test by liunianliang */
#ifdef LENOVO_MMITEST_SUPPORT
		.property_is_writeable = node_is_writeable,
...
/* 20160504 Add for lenovo MMI test by liunianliang */
#ifdef LENOVO_MMITEST_SUPPORT
	bat_data->chg_current = BMT_status.ICharging;
	battery_log(BAT_LOG_FULL, "status_smb = %d, capacity_smb = %d, present_smb = %d,chg_current = %d\n",
			    bat_data->status_smb, bat_data->capacity_smb, bat_data->present_smb,bat_data->chg_current);
#else
...
extern signed int gFG_capacity_by_v;
...
static void battery_update(struct battery_data *bat_data)
{
	...
	if (BMT_status.UI_SOC <= 1) {
       ...
       //huangyisong_add for_runin battery test start tangqingcai sync
               if(g_runin_battery_test_flag)
               {
         ...
         set_rtc_spare_fg_value(BMT_status.SOC);
                   ...
                   if((g_runin_set_soc==KAL_TRUE) && ((abs(gFG_capacity_by_v - BMT_status.UI_SOC )) > 10))
                   {
                       battery_log(BAT_LOG_CRTI, "[BATTERY] runing set_rtc_spare_fg_value =%d\n",gFG_capacity_by_v);
                       set_rtc_spare_fg_value(gFG_capacity_by_v);
                   }else{
...
unsigned int g_tempStatus = 0;
unsigned int g_tempStatusPre = 3;
//CHR_CURRENT_ENUM current=0;
int set_lenovo_charger_status(void)
{
	int temperature = 0;
	if(!g_battery_test_enable)
		temperature = BMT_status.temperature;
	else
	{
		temperature = g_battery_test_temperature;
	}
    ...
    {
        g_BatteryNotifyCode |= 0x0032;
        battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d) less of range\n", BMT_status.temperature);
    }
    else
    {
	...
	g_tempStatus = 0;//no case
	if(temperature <= 0)
	{
		battery_xlog_printk(BAT_LOG_CRTI, "[chenlei2] temperature <= 0\n");
		if(get_charging_setting_current()!=CHARGE_CURRENT_0_00_MA)
		{
			g_tempStatus = 1;
			g_tempStatusPre = g_tempStatus;
			g_BatteryNotifyCode &= ~(0x1000);
		}
	...
	{
			battery_xlog_printk(BAT_LOG_CRTI, "[chenlei2] temperature > 0 && temperature <= 10\n");
			if(get_charging_setting_current()!=CHARGE_CURRENT_550_00_MA)
			{
				g_tempStatus = 2;
				g_tempStatusPre = g_tempStatus;
				if((g_tempStatusPre == 1) || (g_tempStatus == 2))//recovery from under 0,send notefication
				{
					//g_BatteryNotifyCode |= 0x1000;
				}
				else
	...
	else if(temperature > 15 && temperature <= 45)//650
	{
		
			battery_xlog_printk(BAT_LOG_CRTI, "[chenlei2] temperature > 10 && temperature <= 45\n");
		
			if(get_charging_setting_current()!=CHARGE_CURRENT_950_00_MA)
			{
				g_tempStatus = 3;
				g_tempStatusPre = g_tempStatus;
				g_BatteryNotifyCode &= ~(0x1000);
				battery_xlog_printk(BAT_LOG_CRTI, "zoukai check now\n");
			}
		
	}
	else if(temperature > 45 && temperature <= 50)//450
	{
		
			battery_xlog_printk(BAT_LOG_CRTI, "[chenlei2] temperature > 45 && temperature <= 50\n");
			if(get_charging_setting_current()!=CHARGE_CURRENT_950_00_MA)
			{
				g_tempStatus = 4;
				g_tempStatusPre = g_tempStatus;
				if((g_tempStatusPre == 5) || (g_tempStatus == 4))//recovery send notefication
				{
					//g_BatteryNotifyCode |= 0x1000;
				}
				else
	...
	battery_xlog_printk(BAT_LOG_CRTI, "[chenlei2] g_tempStatus= %d,g_tempStatusPre = %d,BMT_status.charger_type= %d\n", g_tempStatus,g_tempStatusPre,BMT_status.charger_type);
	//if(g_tempStatusPre == g_tempStatus || BMT_status.bat_charging_state!=CHR_PRE)
	//	return 0;//no change,return
	//battery_xlog_printk(BAT_LOG_CRTI, "[chenlei2] g_tempStatus=%d\n", g_tempStatus);
	//handle
	if(BMT_status.charger_type == STANDARD_CHARGER)
	{	
	switch(g_tempStatus)
	{
		...
		case 1:
			{
				set_bat_charging_current_limit(0);
			   break;
			}
		case 2:
			{
				set_bat_charging_current_limit(600);
			   break;
			}
		case 3:
			{
				set_bat_charging_current_limit(950);
			   break;
			}
		case 4:
			{
...
void BAT_thread(void)
{
	...
	static bool first_chargefull  = KAL_TRUE;
...
//add by cassy begin
    //printk("BMT_status.UI_SOC=%d,g_platform_boot_mode=%d",BMT_status.UI_SOC,g_platform_boot_mode);

    if (( BMT_status.bat_full == KAL_TRUE )&&(g_platform_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT
     || g_platform_boot_mode == LOW_POWER_OFF_CHARGING_BOOT))
    {
    	//printk("yinyapeng add test by cassy_for key");
     
	     if (first_chargefull == KAL_TRUE) {
		 	     printk("yinyapeng add first_chargefull\n");
			     kpd_pwrkey_pmic_handler(1);//直接调用powerkey事件就可以了。如果不想每十秒一次提醒的话，可以设置flag来判断提醒几次。
			     kpd_pwrkey_pmic_handler(0);
			     first_chargefull = KAL_FALSE;
		}
...
static long adc_cali_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    int icharging_enable = KAL_FALSE;
#endif	
	...
	case BAT_STATUS_READ:
		...
		ret = copy_from_user(battery_in_data, user_data_addr, 4);
            //huangyisong_add_20130312 start for_runin tangqingcai sync
            //3->On  ;  2->Off
            ...
            //huangyisong_add_20130312 end sync end
...
//add by chenlei2
/*
static int battery_test_read(char *buf, char **start, off_t off, int count, int *eof, void *data)
{
    int len = 0;
    char *p = buf;
    
    p += sprintf(p, "g_battery_test_enable=%d,\ng_battery_test_temperature=%d,\ng_tempStatus=%x,\ng_tempStatusPre=%x,\ng_BatteryNotifyCode=%x\n", 
        g_battery_test_enable,g_battery_test_temperature,g_tempStatus,g_tempStatusPre,g_BatteryNotifyCode);
...
/*
static ssize_t battery_test_write(struct file *file, const char *buffer, unsigned long count, void *data)
{
    int len = 0, bat_tt_enable=0, bat_thr_test_temp=0;
    char desc[32];
    ...
    if (sscanf(desc, "%d %d", &bat_tt_enable, &bat_thr_test_temp) == 2)
    {

        g_battery_test_enable = bat_tt_enable;
		g_battery_test_temperature = bat_thr_test_temp;
        battery_xlog_printk(BAT_LOG_CRTI, "bat_tt_enable=%d, bat_thr_test_mode=%d\n",g_battery_test_enable,g_battery_test_temperature);
        
        return count;
    }
    else
    {
        battery_xlog_printk(BAT_LOG_CRTI, "  bad argument, echo [bat_tt_enable] [bat_thr_test_temp] [bat_thr_test_temp] > battery_cmd\n");
    }
    
    return -EINVAL;
}  */

static int mt_batteryTest_probe(struct platform_device *dev)    
{
    int ret_device_file = 0;
    struct proc_dir_entry *entry = NULL;
    struct proc_dir_entry *battery_dir = NULL;

    battery_xlog_printk(BAT_LOG_CRTI, "******** mt_batteryTest_probe!! ********\n" );
    ...
    battery_dir = proc_mkdir("mtk_battery_test", NULL);
    if (!battery_dir)
    {
        pr_err("[%s]: mkdir /proc/mtk_battery_test failed\n", __FUNCTION__);
    }
    else
    {
       // entry = create_proc_entry("battery_test", S_IRUGO | S_IWUSR, battery_dir);
        if (entry)
        {
        //    entry->read_proc = battery_test_read;
        //    entry->write_proc = battery_test_write;
        }
    }

    battery_xlog_printk(BAT_LOG_CRTI, "******** mtk_battery_test!! ********\n" );    
...
static int proc_utilization_set_soc_show(struct seq_file *m, void *v)
{
	seq_printf(m, "=> g_runin_set_soc =%u\n",
		   g_runin_set_soc);
	return 0;
}

static int proc_utilization_set_soc_open(struct inode *inode, struct file *file)
{
	return single_open(file, proc_utilization_set_soc_show, NULL);
}

static ssize_t set_soc_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	int len = 0;
	char desc[32];
    unsigned int set_soc;
    ...
    desc[len] = '\0';
    if (sscanf(desc, "%u", &set_soc) == 1) {
		battery_log(BAT_LOG_CRTI, "[Battery] set_soc_write : %u\n", set_soc);
        if (set_soc == 1) {
            g_runin_set_soc=KAL_TRUE;
            battery_log(BAT_LOG_CRTI, "[BATTERY] runing set_rtc_spare_fg_value =%d\n",gFG_capacity_by_v);
            set_rtc_spare_fg_value(gFG_capacity_by_v);
        } else if (set_soc == 0) {
            g_runin_set_soc=KAL_FALSE;
        }

        return count;
    } 

	battery_log(BAT_LOG_CRTI, "[Battery] set_soc_write fail: %u\n", g_runin_set_soc);

	return -EINVAL;
}

static const struct file_operations set_soc_by_v_proc_fops = {
	.open = proc_utilization_set_soc_open,
	.read = seq_read,
	.write = set_soc_write,
};
...
static ssize_t current_cmd_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	...
	if (sscanf(desc, "%d %d", &cmd_current_unlimited, &cmd_discharging) == 2) {
		...
		//add by liunianliang for runin ,when the battery ui.soc is 100%, it wouldn't change. 2016.05.14
...
static int mt_batteryNotify_probe(struct platform_device *dev)
{
	...
	if (!battery_dir) {
		...
		proc_create("current_cmd", S_IRUGO | /*S_IWUSR*/S_IWUGO, battery_dir, &current_cmd_proc_fops); //modify by liunianliang for runin write
		...
		proc_create("set_soc_by_v", S_IRUGO | S_IWUGO, battery_dir,
			    &set_soc_by_v_proc_fops);
		battery_log(BAT_LOG_CRTI, "proc_create set_soc_by_v_proc_fops\n");
#else
...
//add by chenlei2
struct platform_device MT_batteryTest_device = {
    .name   = "mt-battery-test",
    .id        = -1,
};

static struct platform_driver mt_batteryTest_driver = {
    .probe        = mt_batteryTest_probe,
    .driver     = {
        .name = "mt-battery-test",
    },
};
static int __init battery_init(void)
{
	...
	if (ret) {
		...
		//add by chenlei2
    ret = platform_device_register(&MT_batteryTest_device);
    if (ret) {
        battery_xlog_printk(BAT_LOG_CRTI, "****[mt_batteryNotify] Unable to device register(%d)\n", ret);
        return ret;
    }    
   ret = platform_driver_register(&mt_batteryTest_driver);
    if (ret) {
