...
#if defined(MTK_CL_BCCT_SET_CURRENT_LIMIT)
extern int g_cl_bcct_cur_limit_is_set;
#endif
...
int set_lenovo_charger_status(void)
{
	...
	else if(temperature > 0 && temperature <= 15)
	{
			...
			if(get_charging_setting_current()!=CHARGE_CURRENT_450_00_MA)
			{
				g_tempStatus = 2;
				g_tempStatusPre = g_tempStatus;
	...
	else if(temperature > 15 && temperature <= 45)//700
	{
		
			battery_xlog_printk(BAT_LOG_CRTI, "[chenlei2] temperature > 10 && temperature <= 45\n");
		
			if(get_charging_setting_current()!=CHARGE_CURRENT_700_00_MA)
			{
				g_tempStatus = 3;
				g_tempStatusPre = g_tempStatus;
				g_BatteryNotifyCode &= ~(0x1000);
	...
	else if(temperature > 45 && temperature <= 50)//700
	{
	...
	switch(g_tempStatus)
	{
		...
		case 2:
			{
				set_bat_charging_current_limit(500);
			   break;
			}
		case 3:
			{
				set_bat_charging_current_limit(750);
			   break;
			}
		case 4:
			{
...
/* HQ:zhangjingzhi on: Tue, 26 Apr 2016 11:19:27 +0800
