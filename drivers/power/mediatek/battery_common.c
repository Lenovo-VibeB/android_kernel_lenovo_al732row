...
//wangguan add proc charging current
...
//add by yinyapeng 20160317 ==>
extern unsigned int  g_lcmSuspend;
	...
	if (BMT_status.bat_full == KAL_TRUE) {	/* charging full first, UI tracking to 100% */
		if (BMT_status.UI_SOC >= 100) {
			...
			if (timer_counter >= (cust_sync_time / BAT_TASK_PERIOD)) {
				...
				timer_counter++;
				if(BMT_status.UI_SOC>=100)//by likai add change HV jump to 100%
...
void mt_battery_GetBatteryData(void)
{
	...
	//temperature = 47;//by likai battery_meter_get_battery_temperature();
	...
	printk("by likai BMT_status.ICharging=%d \n",BMT_status.ICharging);
	...
	if (battery_cmd_thermal_test_mode == 1) {
		...
		BMT_status.temperature =
		    mt_battery_average_method(BATTERY_AVG_TEMP, &batteryTempBuffer[0], temperature,
					      &temperature_sum, batteryIndex);  //by likai
		//BMT_status.temperature=47;//by likai change a temp
	}
...
void do_chrdet_int_task(void)
{
	if (g_bat_init_flag == KAL_TRUE) {
		...
		if (g_battery_soc_ready == KAL_FALSE) {
			...
			BMT_status.SOC = battery_meter_get_battery_percentage();
			printk("'BMT_status.SOC =%d ;  by likai \n",BMT_status.SOC);
		}
...
//add by yinyapeng 20160317 ==>
	printk("yinyapeng add g_call_state = %d\n",g_call_state);
	if (upmu_is_chr_det() == KAL_TRUE) {
		//printk("yinyapeng add g_call_state = %d\n",g_call_state);
		if (g_call_state == 1 && g_lcmSuspend == 1) {
			if (flag_call == 1) {
				 set_bat_charging_current_limit(300);
				 flag_call=2;
			}
		} else {
				if(flag_call == 2) { 
					flag_call = 1;  
					set_bat_charging_current_limit(700);  
  			}  
		}
...
#if 0
	if (g_call_state == CALL_ACTIVE && (upmu_is_chr_det() == KAL_TRUE)) {
	...
	if (g_call_state == 1 && (upmu_is_chr_det() == KAL_TRUE)) {
			set_bat_charging_current_limit(450);
	} else {
			set_bat_charging_current_limit(700);  
	}
...
//add by cassy begin
    ...
    if ((BMT_status.UI_SOC == 100) && ( BMT_status.bat_full == KAL_TRUE)&&(g_platform_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT
     || g_platform_boot_mode == LOW_POWER_OFF_CHARGING_BOOT))
    {
...
//liuxiong add 20131122 start
			...
			else if (*user_data_addr == 4){
					g_runin_set_soc=KAL_TRUE;
					if((abs(gFG_capacity_by_v - BMT_status.UI_SOC)) > 10){
					battery_log(BAT_LOG_CRTI, "[BATTERY] runing set_rtc_spare_fg_value =%d\n",gFG_capacity_by_v);
					set_rtc_spare_fg_value(gFG_capacity_by_v);
					}
			}else if(*user_data_addr == 5){
...
static ssize_t proc_get_charge_current(struct file *file, char *buffer, size_t count, loff_t *ppos)
{
...
static ssize_t proc_set_charge_current(struct file *file,const char *buffer, size_t count, loff_t *ppos)
{
	...
	if (wtire_data[0] == 1)
		g_set_current = 1;
	else if (wtire_data[0] == 2)
		g_set_current = 2;
	else if (wtire_data[0] == 3)
		g_set_current = 3;
	else if (wtire_data[0] == 4)
		g_set_current = 4;
	else if (wtire_data[0] == 5)
		g_set_current = 5;	
	else
		g_set_current = 0;

	printk("proc_set_charge_current: %s, g_set_current = %d,set_hq_charging_current=%d\n", __func__, g_set_current,wtire_data[0]);
...
{
   		{
			.name="charge_current",
			{	.owner = THIS_MODULE,
				.read=proc_get_charge_current,
				.write=proc_set_charge_current,
			} 
		},
};
...
#endif
//wangguan add proc charging current
#if 1 //wangguan add for charge
		proc_hq_dbg_add("charge", ctp_proc_entry, sizeof(ctp_proc_entry)/sizeof(struct hq_dbg_entry));
#endif	
