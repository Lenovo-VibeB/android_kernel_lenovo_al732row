...
static BATTERY_VOLTAGE_ENUM select_jeita_cv(void)
{
	...
	printk(" select_jeita_cv cv_voltage=%d\n",cv_voltage);
	return cv_voltage;
}
...
PMU_STATUS do_jeita_state_machine(void)
{
	...
	if (BMT_status.temperature >= TEMP_POS_60_THRESHOLD) {
		...
		if (((g_temp_status == TEMP_POS_45_TO_POS_60)
			...
			if (batt_cust_data.high_battery_voltage_support)
				g_jeita_recharging_voltage = JEITA_TEMP_POS_10_TO_POS_45_RECHARGE_VOLTAGE;//by likai 4200
			else
...
static void set_jeita_charging_current(void)
{
	...
	if (g_temp_status == TEMP_NEG_10_TO_POS_0) {
		...
		g_temp_CC_value = CHARGE_CURRENT_450_00_MA;	/* 0C ~10C */
		battery_log(BAT_LOG_CRTI, "[BATTERY] JEITA set charging current : %d\r\n",
				    g_temp_CC_value);
	} else if (g_temp_status == TEMP_POS_10_TO_POS_45) {
//add by yinyapeng ==>
		printk("yinyapeng add g_lcmSuspend = %d,g_call_state = %d\n",g_lcmSuspend,g_call_state);
		if (g_call_state == 1  && g_lcmSuspend == 1) {
			printk("yinyapeng set CHARGE_CURRENT_200_00_MA in set_jeita_charging_current\n");
...
//<== end			
		...
		g_temp_CC_value = CHARGE_CURRENT_650_00_MA;	/* 45C ~ 60C */
		battery_log(BAT_LOG_CRTI, "[BATTERY] JEITA set charging current : %d\r\n",
				    g_temp_CC_value);
	}else{
...
extern int g_set_current;
void set_hq_charging_current(void)
{
    if (g_set_current==1){
		g_temp_CC_value = 40000;	/* for low temp */
		printk("[BATTERY] set_hq_charging_current set charging current : %d\r\n",g_temp_CC_value);
    	}

	if (g_set_current==2){
		g_temp_CC_value = 45000;	/* for low temp */
		printk( "[BATTERY] wangguan set_hq_charging_current set charging current : %d\r\n",g_temp_CC_value);
    	}

	if (g_set_current==3){
		g_temp_CC_value = 50000;	/* for low temp */
		printk("[BATTERY] set_hq_charging_current set charging current : %d\r\n",g_temp_CC_value);
    	}

	if (g_set_current==4){
		g_temp_CC_value = 60000;	/* for low temp */
		printk("[BATTERY] set_hq_charging_current set charging current : %d\r\n",g_temp_CC_value);
    	}
	
	if (g_set_current==5){
		g_temp_CC_value = 70000;	/* for low temp */
		printk("[BATTERY] set_hq_charging_current set charging current : %d\r\n",g_temp_CC_value);
    	}
...
//add by yinyapeng ==>
			...
			if (g_call_state == CALL_ACTIVE ) {
				printk("STANDARD_CHARGER g_temp_CC_value = CHARGE_CURRENT_450_00_MA\n");
				g_temp_CC_value = CHARGE_CURRENT_450_00_MA;
			} else {
					printk("STANDARD_CHARGER g_temp_CC_value = ac_charger_current\n");
					g_temp_CC_value = batt_cust_data.ac_charger_current;
			}
		
#endif

				printk("[charge]yinyapeng add STANDARD_CHARGER\n");
				if (g_call_state == 1  &&  g_lcmSuspend == 1) {
					printk("STANDARD_CHARGER g_temp_CC_value = CHARGE_CURRENT_300_00_MA\n");
					g_temp_CC_value = CHARGE_CURRENT_300_00_MA;
				} else {
...
void pchr_turn_on_charging(void)
{
	...
	if (g_set_current)
      	{
		printk("2 g_set_current %d \n",g_set_current);
	      set_hq_charging_current();
      	}
	else
	{
		set_jeita_charging_current();
...
if (ta_check_ta_control == KAL_FALSE)
	...
	printk("by likai g_temp_CC_value=%d\n",g_temp_CC_value);
