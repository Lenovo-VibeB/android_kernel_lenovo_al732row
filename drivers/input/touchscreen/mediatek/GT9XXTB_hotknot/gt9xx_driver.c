...
static ssize_t gesture_switch_write_proc(struct file *file, const char __user *buffer, size_t size, loff_t *ppos);
...
#if (defined(GTP_ESD_PROTECT) || defined(CONFIG_GTP_COMPATIBLE_MODE))
u8 is_resetting = 0;
#endif
...
#if defined(CONFIG_GTP_SLIDE_WAKEUP)  // liguangxian  20140917
static ssize_t gt91xxge_config_read_proc(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
	char *p = page;
...
static s32 gtp_init_panel(struct i2c_client *client)
{
	...
	//hardware_info.tpd_sensor_id = sensor_id;
...
static ssize_t gesture_switch_write_proc(struct file *file, const char __user *buffer, size_t size, loff_t *ppos)
{
	...
	if (copy_from_user( &wtire_data, buffer, size ))
		return -EFAULT;
	...
	do {
        ...
        if (DOZE_ENABLED == doze_status)
        {
            ...
            continue;*/
                if (0xCC == doze_buf[2])
                {
                    GTP_INFO("0XCC come on HAHA!!");
                    doze_status = DOZE_ENABLED;
			 ret = gtp_i2c_read(i2c_client_point, tell_buf, 3);
			 if(tell_buf[2]==2)
			 {
                        input_report_key(tpd->dev, KEY_F12, 1);
                        //input_sync(tpd->dev);
		           input_report_key(tpd->dev, KEY_F12, 0);
                        input_sync(tpd->dev);	
			GTP_INFO("0XCC HELLO Camera!! 0x%02X",tell_buf[2]);
			 }
			 else{
			...
			GTP_INFO("0XCC HELLO Phone!!");
                    ...
                    */
                    ...
                    tell_buf[2]=0x00;
                    doze_buf[2] = 0x00;
			gtp_i2c_write(i2c_client_point, tell_buf, 3);
                    gtp_i2c_write(i2c_client_point, doze_buf, 3);
                }
		   ...
		   else if(doze_buf[2]=='m')
		   	{
		   		GTP_INFO("click m!");
                         doze_status = DOZE_ENABLED;
                        input_report_key(tpd->dev, KEY_F6, 1);
                       // input_sync(tpd->dev);
                        input_report_key(tpd->dev, KEY_F6, 0);
                        input_sync(tpd->dev);
                    // clear 0x814B
                    doze_buf[2] = 0x00;
                    gtp_i2c_write(i2c_client_point, doze_buf, 3);
	   ...
	   if ((finger & 0x80) == 0)
	   {
		...
		if (touch_num) {
			for (i = 0; i < touch_num; i++) {
				...
				GTP_DEBUG("GTP caliberated:[X:%04d, Y:%04d]",
					  input_x, input_y);
...
{
	GTP_DEBUG("entering no doze mode...");
