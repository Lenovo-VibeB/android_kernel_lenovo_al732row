...
static int tpd_enable_ps(int enable)
{
	...
	//printk("[proxi_5206] yinyapeng add tpd_enable_ps\n");
	//printk("[proxi_5206]yinyapeng add read: 999 0xb0's value is 0x%02X\n", state);
	if (enable){
		state |= 0x01;
	...
	ret = fts_write_reg(i2c_client, 0xB0, 0x01);
		...
		void* buff_out, int size_out, int* actualout)
{
	...
	//printk("[proxi_5206] yinyapeng add tpd_ps_operate\n");
	switch (command)
	{
		case SENSOR_DELAY:
...
static int touch_event_handler(void *unused)
{
	...
	do {
		...
		set_current_state(TASK_RUNNING);
			...
			//printk("tpd fts_read_Gestruedata state=%d\n",state);
		 ...
		 #endif
		 		printk("[proxi_5206]touch_event_handler start\n");
#ifdef TPD_PROXIMITY
				 if (tpd_proximity_flag == 1)
				 {
					 ...
					 {
						 tpd_enable_ps(1);
						// printk("[proxi_5206] yinyapeng add state1 is not 0x01\n");
					 }
					 ...
					 if (proximity_status == 0xC0)
					 {
						...
						// printk("proxi_5206 yinyapeng add proximity_status == 0xC0\n");
					 }
					 else if(proximity_status == 0xE0)
					 {
						 tpd_proximity_detect = 1;
						// printk("proxi_5206 yinyapeng add proximity_status == 0xE0\n");
					 }
			...
			if (tpd_dts_data.use_tpd_button) {
				...
				if (finfo.y[0] > TPD_RES_Y) {
					if ((cinfo.p[0] == 0) || (cinfo.p[0] == 2)) {
							...
							//yinyapeng modify
							tpd_down(cinfo.x[0], cinfo.y[0],cinfo.p[0],cinfo.id[0]);
...
//wwm end//
	...
	//printk("frank_zhonghua:tpd->gesture_enable = %d\n",tpd->gesture_enable);
	#if FTS_GESTRUE_EN
	printk("frank_zhonghua:tpd->gesture_enable = %d\n",tpd->gesture_enable);
