...
/* The below .h file are included for MSG21xxA/MSG22xx */
// Modify.
...
/* The below .h file are included for MSG26xxM/MSG28xx */
...
void _ItoTestMsg28xxDebugShowArray(void *pBuf, u16 nLen, int nDataType, int nCarry, int nChangeLine)
{
    u8 * pU8Buf = NULL;
    s8 * pS8Buf = NULL;
    u16 * pU16Buf = NULL;
...
void _ItoTestMsg28xxDebugShowS32Array(s32 *pBuf, u16 nRow, u16 nCol)
{
    int i, j;
...
{    
    ...
    if (nDriMode == 2) // for short test
    {
        ...
        for (i = 0; i < nSF; i++)
        {
            ...
            //_ItoTestMsg28xxDebugShowArray(nShotData, 26, 8, 16, 16);   
    ...
    else // for open test
    {
        ...
        {        
            ...
            //_ItoTestMsg28xxDebugShowArray(nShotData, 390, 8, 10, 16);  
...
{        
    ...
    _ItoTestMsg28xxDebugShowArray(pDeltaC, _gMutualICSenseLineNum * _gMutualICDriveLineNum, -32, 10, _gMutualICSenseLineNum);  
...
static s32 _DrvMpTestItoOpenTestMsg28xxOpenJudge(u16 nItemID, s8 pNormalTestResult[][2], u16 pNormalTestResultCheck[][13]/*, u16 nDriOpening*/)
{
    ...
    //_ItoTestMsg28xxDebugShowArray(_gMutualICResult, nRowNum*nColumnNum, -32, 10, nColumnNum);
    for (j = 0; j < _gMutualICDriveLineNum; j ++)
    {
        for (i = 0; i < _gMutualICSenseLineNum; i ++)
        {
    ...
    for (j = 0; j < nRowNum; j ++)
    {
        ...
        for (i = 0; i < nColumnNum; i ++)
        {
   	   	    if (0 == _DrvMpTestItoTestCheckValueInRange(_gMutualICResult[i * _gMutualICDriveLineNum + j], (s32)(nAvg + nAvg * MSG28XX_DC_RANGE/100), (s32)(nAvg - nAvg * MSG28XX_DC_RANGE/100)))
   	   	    {
                _gMutualICTestFailChannel[i * _gMutualICDriveLineNum + j] = 1;
                _gTestFailChannelCount ++; 
                nRetVal = -1;
...
static ItoTestResult_e _DrvMpTestItoShortTestMsg26xxmJudge(u8 nItemID)
{
    ...
    DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__);
   	...
   	for (i = 0;i < nTestPinLength; i++)
   	{
   	   	...
   	   	if (0 == _DrvMpTestItoTestCheckValueInRange(_gMutualICDeltaC[i], MSG26XXM_SHORT_VALUE, -MSG26XXM_SHORT_VALUE))
   	   	{
   	   	   	nRetVal = ITO_TEST_FAIL;
   	   	   	_gTestFailChannelCount++;
...
static s32 _DrvMpTestMsg28xxItoShortTest(u8 nItemID)
{
    ...
    _ItoTestMsg28xxDebugShowArray(_gMutualICDeltaC, 128, -32, 10, 8);
...
static s32 _DrvMpTestItoShortTestMsg28xxJudge(u8 nItemID, /*s8 pNormalTestResult[][2],*/ u16 pTestPinMap[][13], u16 * pTestPinCount)
{
    ...
    *pTestPinCount = _DrvMpTestItoShortTestMsg28xxReadTestPins(nItemID, nTestPins);
    //_ItoTestMsg28xxDebugShowArray(nTestPins, *pTestPinCount, 16, 10, 8);    
    if (*pTestPinCount == 0)
    {
   ...
   */ 
    ...
    for (i = 0; i < *pTestPinCount; i++)
    {
        pTestPinMap[nItemID][i] = nTestPins[i];    
   	   	if (0 == _DrvMpTestItoTestCheckValueInRange(_gMutualICResult[i + (nItemID - 1) * 13], MSG28XX_SHORT_VALUE, -1000))    //0: false   1: true
        {
    ...
    //_ItoTestMsg28xxDebugShowArray(pTestPinMap[nItemID], *pTestPinCount, 16, 10, 8);
...
_retry_short:
    ...
    _ItoTestMsg28xxDebugShowArray(_gMutualICResult, _gMutualICSenseLineNum + _gMutualICDriveLineNum, -32, 10, 8);
...
s32 _DrvMpTestMsg26xxmItoWaterProofTestEntry(void)
{
	...
	_ItoTestMsg28xxDebugShowArray(_gMutualICResultWater, 12, -32, 10, 8);
	for (i = 0; i < 12; i++)
	{
...
static s32 _DrvMpTestMsg28xxItoWaterProofTest(U32 nDelay)
{
    ...
    _ItoTestMsg28xxDebugShowArray(_gMutualICDeltaCWater, 12, -32, 10, 16);
