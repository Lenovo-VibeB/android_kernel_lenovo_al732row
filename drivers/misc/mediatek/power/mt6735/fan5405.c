...
#if 1//chenjun
static int fan5405_platform_probe(struct platform_device *pdev);
static int fan5405_platform_remove(struct platform_device *pdev);
	
static const struct of_device_id fan5405_platform_of_match[] = {
	{.compatible = "mediatek,chr_stat",},
	{}
};

static struct platform_driver fan5405_platform_driver = {
	.probe	  = fan5405_platform_probe,
	.remove	 = fan5405_platform_remove,
	.driver = {
		.name  = "chr_stat",
	#ifdef CONFIG_OF
		.of_match_table = fan5405_platform_of_match,
	#endif
	}
};
...
struct pinctrl *fan5405_gpio_ctrl = NULL;
struct pinctrl_state *fan5405_chr_enable_1 = NULL;
struct pinctrl_state *fan5405_chr_enable_0 = NULL;
struct pinctrl_state *fan5405_otg_enable_1 = NULL;
struct pinctrl_state *fan5405_otg_enable_0 = NULL;

int fan5405_gpio_set(int PinType,int Val)
{
	int ret = 0;
	...
	case FAN5405_CHR_EN:
			if (Val == 0)
				pinctrl_select_state(fan5405_gpio_ctrl, fan5405_chr_enable_0);
			else
				pinctrl_select_state(fan5405_gpio_ctrl, fan5405_chr_enable_1);
		break;
	case FAN5405_OTG_EN:
			if (Val == 0)
				pinctrl_select_state(fan5405_gpio_ctrl, fan5405_otg_enable_0);
			else
				pinctrl_select_state(fan5405_gpio_ctrl, fan5405_otg_enable_1);
		break;
	default:
...
int fan5405_get_platform_info(struct platform_device *pdev)
{
	int ret;

	fan5405_gpio_ctrl = devm_pinctrl_get(&pdev->dev);
	if (IS_ERR(fan5405_gpio_ctrl)) {
		dev_err(&pdev->dev, "Cannot find disp gpio pinctrl!");
		ret = PTR_ERR(fan5405_gpio_ctrl);
	}
	/*Cam0 Power/Rst Ping initialization */
	fan5405_chr_enable_1 = pinctrl_lookup_state(fan5405_gpio_ctrl, "fan5405_chr_enable_1");
	if (IS_ERR(fan5405_chr_enable_1)) {
		ret = PTR_ERR(fan5405_chr_enable_1);
		pr_debug("%s : pinctrl err, fan5405_chr_enable_1 \n", __func__);
	}

	fan5405_chr_enable_0 = pinctrl_lookup_state(fan5405_gpio_ctrl, "fan5405_chr_enable_0");
	if (IS_ERR(fan5405_chr_enable_0)) {
		ret = PTR_ERR(fan5405_chr_enable_0);
		pr_debug("%s : pinctrl err, fan5405_chr_enable_0\n", __func__);
	}


	fan5405_otg_enable_1 = pinctrl_lookup_state(fan5405_gpio_ctrl, "fan5405_otg_enable_1");
	if (IS_ERR(fan5405_otg_enable_1)) {
		ret = PTR_ERR(fan5405_otg_enable_1);
		pr_debug("%s : pinctrl err, fan5405_otg_enable_1\n", __func__);
	}

	fan5405_otg_enable_0 = pinctrl_lookup_state(fan5405_gpio_ctrl, "fan5405_otg_enable_0");
	if (IS_ERR(fan5405_otg_enable_0)) {
		ret = PTR_ERR(fan5405_otg_enable_0);
		pr_debug("%s : pinctrl err, fan5405_otg_enable_0\n", __func__);
	}
	printk("[fan5405_gpio%d] fan5405_gpio_ctrl----------\n", pdev->id);
	return 0;
}

static int fan5405_platform_probe(struct platform_device *pdev)
{
	fan5405_get_platform_info(pdev);
	return 0;  
}
...
#if 1//chenjun
	if (platform_driver_register(&fan5405_platform_driver) != 0) {
		printk("unable to register fan5405 platform driver.\n");
		return -1;
	}
#endif
