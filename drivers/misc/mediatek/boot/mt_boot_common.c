...
#include <mt-plat/mt_boot.h>//add by liunianliang
...
void init_boot_common(unsigned int line)
{
	...
	int rc;
        ...
        printk("wangguan init_boot_common %d\n",g_boot_mode);
        if(g_boot_mode == 11) //APK Test mode startup
        {
            g_boot_mode = NORMAL_BOOT;
            g_boot_mode_ex = 11;
            printk("wangguan init_boot_common %d\n",g_boot_mode_ex);
        }
        printk("wangguan2 init_boot_common %d\n",g_boot_mode);
...
static int hq_boot_status(struct platform_device *dev)
{
    ...
    printk("**wangguan hq_boot_status_probe!! **\n" );
...
{
    int ret = 0;
    printk("** Hq_boot_status_mod_init!! **\n" );
    printk("** Hq_boot_status_mod_init g_boot_mode_ex!! %d**\n",g_boot_mode_ex);
