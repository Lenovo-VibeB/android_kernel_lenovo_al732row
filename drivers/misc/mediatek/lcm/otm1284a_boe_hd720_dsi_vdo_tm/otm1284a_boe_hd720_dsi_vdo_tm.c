...
/*****disp_gpio begin*******************************/
static int disp_gpio_probe(struct platform_device *pdev);
static int disp_gpio_remove(struct platform_device *pdev);

static struct pinctrl *disp_gpio_ctrl = NULL;
static struct pinctrl_state *disp_gpio_enn_h = NULL;
static struct pinctrl_state *disp_gpio_enn_l = NULL;
static struct pinctrl_state *disp_gpio_enp_h = NULL;
static struct pinctrl_state *disp_gpio_enp_l = NULL;
...
static const struct of_device_id disp_gpio_of_match[] = {
	{.compatible = "mediatek,disp_gpio",},
	{}
};

static struct platform_driver disp_gpio_driver = {
	.probe	  = disp_gpio_probe,
	.remove	 = disp_gpio_remove,
	.driver = {
		.name  = "disp_gpio",
	#ifdef CONFIG_OF
		.of_match_table = disp_gpio_of_match,
	#endif
	}
};

static int disp_get_gpio_info(struct platform_device *pdev)
{
	int ret;

	disp_gpio_ctrl = devm_pinctrl_get(&pdev->dev);
	if (IS_ERR(disp_gpio_ctrl)) {
		dev_err(&pdev->dev, "Cannot find disp gpio pinctrl!");
		ret = PTR_ERR(disp_gpio_ctrl);
	}
	/*Cam0 Power/Rst Ping initialization */
	disp_gpio_enn_h = pinctrl_lookup_state(disp_gpio_ctrl, "disp_gpio_enn_h");
	if (IS_ERR(disp_gpio_enn_h)) {
		ret = PTR_ERR(disp_gpio_enn_h);
		pr_debug("%s : pinctrl err, disp_gpio_enn_h \n", __func__);
	}

	disp_gpio_enn_l = pinctrl_lookup_state(disp_gpio_ctrl, "disp_gpio_enn_l");
	if (IS_ERR(disp_gpio_enn_l)) {
		ret = PTR_ERR(disp_gpio_enn_l);
		pr_debug("%s : pinctrl err, disp_gpio_enn_l\n", __func__);
	}


	disp_gpio_enp_h = pinctrl_lookup_state(disp_gpio_ctrl, "disp_gpio_enp_h");
	if (IS_ERR(disp_gpio_enp_h)) {
		ret = PTR_ERR(disp_gpio_enp_h);
		pr_debug("%s : pinctrl err, disp_gpio_enp_h\n", __func__);
	}

	disp_gpio_enp_l = pinctrl_lookup_state(disp_gpio_ctrl, "disp_gpio_enp_l");
	if (IS_ERR(disp_gpio_enp_l)) {
		ret = PTR_ERR(disp_gpio_enp_l);
		pr_debug("%s : pinctrl err, disp_gpio_enp_l\n", __func__);
	}
	printk("[disp_gpio%d] disp_gpio_pinctrl----------\n", pdev->id);
	return 0;
}

int disp_gpio_set(int PinType,int Val)
{
	int ret = 0;
	...
	case DISP_ENN:
			if (Val == 0)
				pinctrl_select_state(disp_gpio_ctrl, disp_gpio_enn_l);
			else
				pinctrl_select_state(disp_gpio_ctrl, disp_gpio_enn_h);
		break;
	case DISP_ENP:
			if (Val == 0)
				pinctrl_select_state(disp_gpio_ctrl, disp_gpio_enp_l);
			else
				pinctrl_select_state(disp_gpio_ctrl, disp_gpio_enp_h);
		break;
	default:
		printk("PinType(%d) is invalid !!\n", PinType);
		break;
	};

	printk("PinType(%d)  val(%d)\n", PinType, Val);
...
static int __init disp_gpio_device_init(void)
{
	printk("MediaTek disp gpio driver init\n");
	if (platform_driver_register(&disp_gpio_driver) != 0) {
		printk("unable to register disp gpio driver.\n");
		return -1;
	}
	return 0;
}
...
static void __exit disp_gpio_device_exit(void)
{
	printk("MediaTek disp gpio driver exit\n");
	/* input_unregister_device(tpd->dev); */
	platform_driver_unregister(&disp_gpio_driver);
}

late_initcall(disp_gpio_device_init);
module_exit(disp_gpio_device_exit);

MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("MediaTek disp gpio driver");
...
static void lcm_get_params(LCM_PARAMS *params)
{
		...
		params->dsi.horizontal_backporch				= 34;//34;//44;//44;//42;
		params->dsi.horizontal_frontporch				= 24;//24;//44;
