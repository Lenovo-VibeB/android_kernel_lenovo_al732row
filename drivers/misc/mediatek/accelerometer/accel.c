...
#include <linux/fs.h>     //by  liunianliang
...
char Accname[20]= "NULL";//by liunianliang
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	printk("gsensor name is %s by acc_show_sensordevnum\n",devname);//by liunianliang
	ret = sscanf(devname+5, "%d", &devnum);
	return snprintf(buf, PAGE_SIZE, "%d\n", devnum);
}

//add by liunianliang for gsensor name begin
/*static ssize_t acc_show_sensorname(struct device* dev,
                                 struct device_attribute *attr, char *buf)
{
	printk("acc name is %s by qaz \n",Accname);
	return sprintf(buf, "%s\n",Accname);
}*/

static struct proc_dir_entry *fts_proc_Gsensor_name=NULL;

static ssize_t fts_Gsensor_name_read( struct file *file, char __user *buffer, size_t count, loff_t *ppos )
{
        char *page = NULL;
        char *ptr = NULL;
        int len, err = -1;
         ...
         ptr = page;
         ptr += sprintf(ptr, "Gsensorname=%s",  Accname);
...
static const struct file_operations gt_gsensor_name_proc_fops = {
    .write = NULL,
    .read = fts_Gsensor_name_read,
};

static int fts_create_proc_gsensor_name(void)
{
       fts_proc_Gsensor_name = proc_create("hq_gsensor", S_IRUGO, NULL,&gt_gsensor_name_proc_fops);
       if (fts_proc_Gsensor_name == NULL)
               {
                       printk("create_proc_entry  hq_gsensor failed\n");
               }
       return 0;
}
//add by liunianliang for gsensor name end
...
static int acc_real_driver_init(void)
{
	...
	printk(" before acc Accname is %s by qaz\n",Accname); //liunianliang
		...
		if (0 != gsensor_init_list[i]) {
			...
			if (0 == err) {
				ACC_LOG(" acc real driver %s probe ok\n",
					gsensor_init_list[i]->name);
				memcpy(Accname, gsensor_init_list[i]->name, strlen(gsensor_init_list[i]->name));//by liunianliang
				break;
			}
		}
	}
	...
	if (i == MAX_CHOOSE_G_NUM) {
		...
		fts_create_proc_gsensor_name();
		}
	printk("after acc Accname is %s by lnl\n",Accname); //liunianliang
	return err;
}
