...
/*****Eur Amera earphone adjust****/
static int accdet_switch = 0;
static int swap_4732 = 0;
static int j = 2;
	/*********************************/
static atomic_t time_counter = ATOMIC_INIT(0);
#endif 
int cur_key = 0;
...
#define ACCDET_DEBUG(format, args...) pr_err(format, ##args)
#define ACCDET_INFO(format, args...) pr_err(format, ##args)
...
#ifdef CONFIG_ACCDET_PIN_SWAP
//===================================================
#define SWITCH_MICBIAS_TIMER   (HZ/2)         // 0.5 seconds
struct timer_list switch_micbias_timer;
static void switch_micbias(unsigned long b);
struct pinctrl *accdet_switch_output_ctrl = NULL;
//struct pinctrl_state *accdet_switch_output_default = NULL;
struct pinctrl_state *accdet_switch_output_l = NULL;
struct pinctrl_state *accdet_switch_output_h = NULL;
//===================================================
#endif
...
#ifdef CONFIG_ACCDET_PIN_SWAP
static int accdet_switch_gpio_setup(struct platform_device *accdet_device);
#endif 
...
/*=======================   yinyapeng add for proc   ===================================*/
static ssize_t proc_get_accdet_current(struct file *file,char *buffer, size_t count, loff_t *ppos);
static ssize_t proc_set_accdet_current(struct file *file, const char *buffer, size_t count, loff_t *ppos);

#if 1 //linmingchuang add for charge
...
{
    {
		...
		{       
			.owner = THIS_MODULE,
			.read = proc_get_accdet_current,
			.write = proc_set_accdet_current,
		} 
     },
}; 
...
/***************yinyapeng add start ===>*********************/
#if 1
//linmingchuang add proc charging current
...
//	tmp[50]="1\0";
	 ...
	 if(swap_4732==1)
	 	{
     	strcpy(buffer, "1");  //	 tmp="1";
	 	 }
	 else
	 	{
	   strcpy(buffer, "0"); // tmp="0";
	  }
...
*/
	int ret;
	ret = pinctrl_select_state(accdet_switch_output_ctrl, accdet_switch_output_h);
	if (ret) {
		printk("%s :[Accdet] pinctrl err, accdet_switch_gpio_h \n", __func__);
	}
       swap_4732 = 1;
       printk("ACCDET accdet_FSA8049_enable \n");
}
...
*/
	int ret;
	ret = pinctrl_select_state(accdet_switch_output_ctrl, accdet_switch_output_l);
	if (ret) {
		printk("%s :[Accdet] pinctrl err, accdet_switch_gpio_l \n", __func__);
	}
	swap_4732 = 0;  
       printk("ACCDET accdet_FSA8049_disable \n");
}
...
static void switch_micbias(unsigned long b)
{
	  ...
	  int i_current_status;
        while(((pmic_pwrap_read(ACCDET_IRQ_STS) & IRQ_STATUS_BIT) && i<10)) {
               ...
               i_current_status = ((pmic_pwrap_read(ACCDET_STATE_RG) & 0xc0)>>6); //A=bit1; B=bit0
		ACCDET_DEBUG("[Accdet] i_current_status is %d\n",i_current_status);
               if (i_current_status == 0)
        {
               if(swap_4732 == 0){
               ...
               if(atomic_read(&time_counter) < 10){
                       
                       switch_micbias_timer.expires = jiffies + (HZ/2);
                       add_timer(&switch_micbias_timer);
                       atomic_inc(&time_counter);
                       printk("[ACCDET] stevej = %d\n",j++);
                       }
...
/*Disable ACCDET*/
		...
		del_timer_sync(&micbias_timer);
#ifdef CONFIG_ACCDET_PIN_SWAP
		del_timer_sync(&switch_micbias_timer);
#endif
#ifdef CONFIG_ACCDET_PIN_RECOGNIZATION
		show_icon_delay = 0;
		cable_pin_recognition = 0;
...
#ifdef CONFIG_ACCDET_PIN_SWAP
          //=======================================================
               init_timer(&switch_micbias_timer);
               switch_micbias_timer.expires = jiffies + SWITCH_MICBIAS_TIMER;
               switch_micbias_timer.function = &switch_micbias;
               switch_micbias_timer.data = ((unsigned long) 0 );
               add_timer(&switch_micbias_timer);
        //=======================================================
#endif
	}
...
static int accdet_switch_gpio_setup(struct platform_device *accdet_device)
{
	...
	accdet_switch_output_ctrl = devm_pinctrl_get(&accdet_device->dev);
	if (IS_ERR(accdet_switch_output_ctrl)) {
		dev_err(&accdet_device->dev, "[Accdet] Cannot find accdet_switch pinctrl!");
		ret = PTR_ERR(accdet_switch_output_ctrl);
		return ret;
	}
	
	accdet_switch_output_l = pinctrl_lookup_state(accdet_switch_output_ctrl, "state_switch_outputl");	
	if (IS_ERR(accdet_switch_output_l)) {
		ret = PTR_ERR(accdet_switch_output_l);
		dev_err(&accdet_device->dev,"%s :[Accdet] pinctrl err, accdet_switch_gpio_l \n", __func__);
		return ret;
	}

	accdet_switch_output_h = pinctrl_lookup_state(accdet_switch_output_ctrl, "state_switch_outputh");	
	if (IS_ERR(accdet_switch_output_h)) {
		ret = PTR_ERR(accdet_switch_output_h);
		dev_err(&accdet_device->dev,"%s :[Accdet] pinctrl err, accdet_switch_gpio_h \n", __func__);
		return ret;
	}

	ret = pinctrl_select_state(accdet_switch_output_ctrl, accdet_switch_output_l);
	if (ret) {
		dev_err(&accdet_device->dev, "[Accdet] failed to activate default pinctrl state as low!\n");
		return ret;
	}
	ACCDET_INFO("[Accdet] GPIO setting up OK !!\n");
	return 1;
}
#endif
...
//yinyapeng add in accdet_probe ==>	
	 proc_hq_dbg_add1("accdet", ctp_proc_entry, sizeof(ctp_proc_entry)/sizeof(struct hq_dbg_entry));
//<==end
