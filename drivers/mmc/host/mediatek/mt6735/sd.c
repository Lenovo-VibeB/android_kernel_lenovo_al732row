...
static int g_power_tune_time = 0;
...
static u32 msdc_power_tuning(struct msdc_host *host)
{
	...
	if (!mmc)
		return 1;
	pr_err("checkme:*****enter power tuning :%d\n",g_power_tune_time);
	...
	if (mmc_card_sd(card) && (host->hw->host_function == MSDC_SD)) {
		if ((g_power_tune_time < MSDC_MAX_POWER_CYCLE)
			&& (host->power_cycle_enable)) {
			/* power cycle */
			g_power_tune_time++;
			ERR_MSG("the %d time, Power cycle start", g_power_tune_time);
			spin_unlock(&host->lock);
#ifdef FPGA_PLATFORM
			hwPowerDown_fpga();
#else
			...
			if ((host->sclk > 100000000) && (g_power_tune_time >= 1))
			...
			host->error = host_err;
		  pr_err("checkme:*****runing power tuning :%d\n",g_power_tune_time);
			if (!ret)
				host->power_cycle_enable = 0;
			ERR_MSG("the %d time, Power cycle Done, host->error(0x%x), ret(%d)",
				g_power_tune_time, host->error, ret);
		} else {
			ERR_MSG("[%d] > max power tune fail request count %d,remove bad card",
    ...
    pr_err("checkme:*****leave power tuning :%d\n",g_power_tune_time);
				...
				if (!(host->sw_timeout) && (host->hw->host_function == MSDC_SD)
					...
					ERR_MSG
					    ...
					    "SW timeout(%d) or read timeout tune(%d),Power cycle",
					     ...
					     host->read_timeout_uhs104);
					     if(host->read_time_tune > 5)
					     {
					    		ERR_MSG("return ENOMEDIUM");
